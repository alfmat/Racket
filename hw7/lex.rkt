...
(define (let-token arg)
  (token 'LET))

(define (define-token arg)
  (token 'DEFINE))

(define (class-token arg)
  (token 'CLASS))

(define (new-token arg)
  (token 'NEW))

(define (send-token arg)
  (token 'SEND))

(define (super-token arg)
  (token 'SUPER))

(define (set-token arg)
  (token 'SET))

(define (lambda-token arg)
  (token 'LAMBDA))
...
(define re-table
  (list
    ...
    (list #rx"^\"[^\"\\\\]*(\\\\.[^\"\\\\]*)*\"(?=[\r\n\t (){},;.]|$)"
          string-token)
    (list #rx"^let" let-token)
    (list #rx"^define" define-token)
    (list #rx"^lambda" lambda-token)
    (list #rx"^class" class-token)
    (list #rx"^new" new-token)
    (list #rx"^send" send-token)
    (list #rx"^super" super-token)
    (list #rx"^set" set-token)
    (list #rx"^[^(){},;.\" \r\n\t0-9][^(){},;.\" \r\n\t]*(?=[\r\n\t (){},;.]|$)"
          name-token)))
